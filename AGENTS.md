...
- Isometric Perspective direction layout is a world setting. It maps Foundry canvas movement axes back to source-art facings and arranges source previews as a compact large diamond made from diamond cells. Source flags and row meanings stay unchanged: bottom `down` represents game southwest, while lower-right `down-right` represents game south.
...
- Enable the world isometric layout and confirm movement southwest/south selects source Down/Down Right; both HUD modes should use closely packed regular-diamond cells with those source positions at the large diamond's bottom/lower-right.
